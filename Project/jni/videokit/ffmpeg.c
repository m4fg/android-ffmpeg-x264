...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    if (is_last_report) {
        ...
        LOGI("\n");
        LOGI("video:%1.0fkB audio:%1.0fkB subtitle:%1.0f global headers:%1.0fkB muxing overhead %f%%\n",
               video_size / 1024.0,
               audio_size / 1024.0,
            ...
            if (!ost->filter &&
                (codec->codec_type == AVMEDIA_TYPE_VIDEO ||
                 codec->codec_type == AVMEDIA_TYPE_AUDIO)) {
                    ...
                    if (r = configure_filtergraph(fg)) {
                        LOGE("Error opening filters! %i\n", r);
                        exit_program(1);
                    }
            }
 ...
 dump_format:
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy)
            ...
            LOGI(" (%s -> %s)", input_streams[ost->source_index]->dec ?
...
static int transcode_step(void)
{
    ...
    if (!ost) {
        ...
        LOGV( "No more inputs to read from, finishing.\n");
        return AVERROR_EOF;
    }
...
static int transcode(void)
{
    ...
    while (!received_sigterm) {
        ...
        if (!need_output()) {
            LOGV( "No more output streams to write to, finishing.\n");
            break;
        }
