                                ...
                                AVSubtitleRect *r)
{
    ...
    if (r->type != SUBTITLE_BITMAP) {
        LOGW("sub2video: non-bitmap subtitle\n");
        return;
    }
    if (r->x < 0 || r->x + r->w > w || r->y < 0 || r->y + r->h > h) {
        LOGW("sub2video: rectangle overflowing\n");
        return;
    }
...
static void ffmpeg_cleanup(int ret)
{
    ...
    if (received_sigterm) {
        LOGI("Received signal %d: terminating.\n",
               (int) received_sigterm);
    } else if (ret && transcode_init_done) {
        LOGI("Conversion failed!(%i)\n", ret);
    }
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    while (bsfc) {
        ...
        if (a > 0) {
            ...
            LOGE("Failed to open bitstream filter %s for stream %d with codec %s",
                         ...
                         AVFrame *frame)
{
    ...
    if (avcodec_encode_audio2(enc, &pkt, frame, &got_packet) < 0) {
        LOGE("Audio encoding failed (avcodec_encode_audio2)\n");
        exit_program(1);
    }
                            ...
                            AVSubtitle *sub)
{
    ...
    if (sub->pts == AV_NOPTS_VALUE) {
        LOGE("Subtitle packets must have a pts\n");
        if (exit_on_error)
            exit_program(1);
        return;
    }
                         ...
                         AVFrame *next_picture)
{
    ...
    if (delta0 < 0 &&
        ...
        LOGW("Past duration %f too large\n", -delta0);
        sync_ipts += cor;
        duration -= cor;
        delta0 += cor;
    }
    ...
    case VSYNC_VSCFR:
        if (ost->frame_number == 0 && delta - duration >= 0.5) {
            LOGD("Not duplicating %d initial frames\n", (int)lrintf(delta - duration));
            delta = duration;
            delta0 = 0;
    ...
    if (nb_frames > (nb0_frames && ost->last_droped) + (nb_frames > nb0_frames)) {
        if (nb_frames > dts_error_threshold * 30) {
            LOGE("%d frame duplication too large, skipping\n", nb_frames - 1);
            nb_frames_drop++;
            return;
        }
  ...
  for (i = 0; i < nb_frames; i++) {
    ...
    if (s->oformat->flags & AVFMT_RAWPICTURE &&
        ...
        if (forced_keyframe) {
            ...
            LOGD("Forced keyframe at time %f\n", pts_time);
        }

        update_benchmark(NULL);
        if (debug_ts) {
            LOGI("encoder <- type:video "
                   "frame_pts:%s frame_pts_time:%s time_base:%d/%d\n",
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        while (1) {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (debug_ts) {
                    LOGI("filter -> pts:%s pts_time:%s time_base:%d/%d\n",
...
static void print_final_stats(int64_t total_size)
{
    ...
    LOGI("video:%1.0fkB audio:%1.0fkB subtitle:%1.0fkB other streams:%1.0fkB global headers:%1.0fkB muxing overhead: ",
           video_size / 1024.0,
           audio_size / 1024.0,
    ...
    for (i = 0; i < nb_input_files; i++) {
        ...
        for (j = 0; j < f->nb_streams; j++) {
            ...
            LOGI( "  Input stream #%d:%d (%s): ",
                   i, j, media_type_string(type));
            LOGI( "%"PRIu64" packets read (%"PRIu64" bytes); ",
                   ist->nb_packets, ist->data_size);

            if (ist->decoding_needed) {
                LOGI( "%"PRIu64" frames decoded",
                       ist->frames_decoded);
                if (type == AVMEDIA_TYPE_AUDIO)
                    LOGI( " (%"PRIu64" samples)", ist->samples_decoded);
                LOGI( "; ");
            }
        ...
        LOGI( "  Total: %"PRIu64" packets (%"PRIu64" bytes) demuxed\n",
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        for (j = 0; j < of->ctx->nb_streams; j++) {
            ...
            LOGI( "  Output stream #%d:%d (%s): ",
                   i, j, media_type_string(type));
            if (ost->encoding_needed) {
                LOGI( "%"PRIu64" frames encoded",
                       ost->frames_encoded);
                if (type == AVMEDIA_TYPE_AUDIO)
                    LOGI( " (%"PRIu64" samples)", ost->samples_encoded);
                LOGI( "; ");
            }

            LOGI( "%"PRIu64" packets muxed (%"PRIu64" bytes); ",
                   ost->packets_written, ost->data_size);

            LOGI( "\n");
        }

        LOGI( "  Total: %"PRIu64" packets (%"PRIu64" bytes) muxed\n",
               total_packets, total_size);
    }
    if(video_size + data_size + audio_size + subtitle_size + extra_size == 0){
        LOGW("Output file is empty, nothing was encoded (check -ss / -t / -frames parameters if used)\n");
    }
}
...
int guess_input_channel_layout(InputStream *ist)
{
    ...
    if (!dec->channel_layout) {
        ...
        LOGW("Guessed Channel Layout for  Input Stream "
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (resample_changed) {
        ...
        if (!guess_input_channel_layout(ist)) {
            LOGE("Unable to find default channel "
        ...
        for (i = 0; i < nb_filtergraphs; i++)
            if (ist_in_filtergraph(filtergraphs[i], ist)) {
                ...
                if (configure_filtergraph(fg) < 0) {
                    LOGE("Error reinitializing filters!\n");
                    exit_program(1);
                }
            }
    }
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (*got_output && ret >= 0) {
        if (ist->dec_ctx->width  != decoded_frame->width ||
            ...
            LOGD("Frame parameters mismatch context %d,%d,%d != %d,%d,%d\n",
                decoded_frame->width,
    ...
    if (debug_ts) {
        LOGI("decoder -> ist_index:%d type:video "
        ...
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_DVB_SUBTITLE &&
           (ist->decoding_needed & DECODING_FOR_OST)) {
            ...
            if (ist->decoding_needed & DECODING_FOR_FILTER)
                LOGW("Warning using DVB subtitles for filtering and output at the same time is not fully supported, also see -compute_edt [0|1]\n");
        }
                                    ...
                                    AVCodecContext *avctx)
{
    ...
    if (!pts) {
        LOGE("Could not allocate forced key frames array.\n");
        exit_program(1);
    }
...
static int transcode_init(void)
{
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if (!oc->nb_streams && !(oc->oformat->flags & AVFMT_NOSTREAMS)) {
            ...
            LOGE("Output file #%d does not contain any stream\n", i);
            return AVERROR(EINVAL);
        }
    }
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy) {
            ...
            case AVMEDIA_TYPE_AUDIO:
                if (audio_volume != 256) {
                    LOGE("-acodec copy and -vol are incompatible (frames are not decoded)\n");
                    exit_program(1);
                }
                ...
                if (!strcmp(ost->enc->name, "libx264")) {
                    ...
                    if (enc_ctx->flags & CODEC_FLAG_PASS2) {
                        ...
                        if (cmdutils_read_file(logfilename, &logbuffer, &logbuffer_size) < 0) {
                            LOGE("Error reading log file '%s' for pass-2 encoding\n",
                                   logfilename);
                            exit_program(1);
                        }
                    ...
                    if (enc_ctx->flags & CODEC_FLAG_PASS1) {
                        ...
                        if (!f) {
                            LOGE("Cannot write log file '%s' for pass-1 encoding: %s\n",
...
//         assert_avoptions(output_files[i]->opts);
 ...
 dump_format:
    ...
    LOGI("===Stream mapping input %i:\n",nb_input_streams);
    for (i = 0; i < nb_input_streams; i++) {
        LOGI("Stream mapping input(%i):\n", i);
        ist = input_streams[i];
        LOGI("Stream mapping input(%i) -> %i:\n", i, ist->nb_filters);
        for (j = 0; j < ist->nb_filters; j++) {
            if (ist->filters[j]->graph->graph_desc) {
                ...
                if (nb_filtergraphs > 1)
                    LOGI(" (graph %d)", ist->filters[j]->graph->index);
                LOGI("\n");
            }
        }
    }
    LOGI("===Stream mapping output %i:\n",nb_output_streams);
    for (i = 0; i < nb_output_streams; i++) {
        LOGI("Stream mapping output(%i):\n", i);
        ost = output_streams[i];
        ...
        if (ost->filter && ost->filter->graph->graph_desc) {
            ...
            LOGI("  %s", ost->filter->name);
            if (nb_filtergraphs > 1)
                LOGI(" (graph %d)", ost->filter->graph->index);
...
static int init_input_threads(void)
{
    ...
    for (i = 0; i < nb_input_files; i++) {
        ...
        if ((ret = pthread_create(&f->thread, NULL, input_thread, f))) {
            LOGE("pthread_create failed: %s. Try to increase `ulimit -v` or decrease `ulimit -s`.\n", strerror(ret));
            av_thread_message_queue_free(&f->in_thread_queue);
            return AVERROR(ret);
        }
    }
...
static int process_input(int file_index)
{
    ...
    if (debug_ts) {
        LOGI("demuxer -> ist_index:%d type:%s "
    ...
    if ((ist->dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
        ...
        if (is->iformat->flags & AVFMT_TS_DISCONT) {
            ...
            if ( delta < -1LL*dts_error_threshold*AV_TIME_BASE ||
                ...
                LOGW("DTS %"PRId64", next:%"PRId64" st:%d invalid dropping\n", pkt.dts, ist->next_dts, pkt.stream_index);
                pkt.dts = AV_NOPTS_VALUE;
            }
            if (pkt.pts != AV_NOPTS_VALUE){
                ...
                if ( delta < -1LL*dts_error_threshold*AV_TIME_BASE ||
                     delta >  1LL*dts_error_threshold*AV_TIME_BASE) {
                    LOGW("PTS %"PRId64", next:%"PRId64" invalid dropping st:%d\n", pkt.pts, ist->next_dts, pkt.stream_index);
                    pkt.pts = AV_NOPTS_VALUE;
                }
            }
        }
    ...
    if (debug_ts) {
        LOGI("demuxer+ffmpeg -> ist_index:%d type:%s pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
               ifile->ist_index + pkt.stream_index, av_get_media_type_string(ist->dec_ctx->codec_type),
               av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
               av_ts2str(pkt.dts), av_ts2timestr(pkt.dts, &ist->st->time_base),
               av_ts2str(input_files[ist->file_index]->ts_offset),
    ...
    if (ret < 0) {
        LOGE("Error while decoding stream #%d:%d: %s\n",
               ist->file_index, ist->st->index, av_err2str(ret));
        if (exit_on_error)
            exit_program(1);
    }
...
static int transcode_step(void)
{
    ...
    if (!ost) {
        ...
        LOGI( "No more inputs to read from, finishing.\n");
        return AVERROR_EOF;
    }
...
static int transcode(void)
{
    ...
    while (!received_sigterm) {
        ...
        if (!need_output()) {
            LOGI( "No more output streams to write to, finishing.\n");
            break;
        }
...
//     }
    ...
    LOGI("received_nb_signals %i", received_nb_signals);
